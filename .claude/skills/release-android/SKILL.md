...
- `preview` profile in `eas.json` sets `"buildType": "apk"` (installable file). The default/production profile would produce an .aab (Play Store format) — not what we want.
...
- **`versionCode`**: managed remotely (`appVersionSource: "remote"` in eas.json). Preview builds keep versionCode as-is; if Android ever refuses to install an update ("app not installed"), bump `expo.version` in app.json and/or check versionCode collision.
