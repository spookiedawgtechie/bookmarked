...
- **Runtime Monet-style themes are the identity.** `lib/theme.tsx` owns five palettes (Sage, Ink, Ocean, Plum, Ember) and four appearance choices (System, Light, tinted Dark, pure-black AMOLED). Screens must call `useTheme()` for colours and `useThemedStyles(createStyles)` for styles; module-scope colour snapshots will not react to a theme change. Never hardcode screen colours.
- Existing libraries with no saved theme default to Ink + AMOLED; an empty new library defaults to Ink + System. Theme settings are device-local in `app_settings` and deliberately excluded from backup.
- AMOLED means `#000000` for background and card surfaces; use borders, spacing, covers, and the selected palette accents for hierarchy. Light/Dark use palette-tinted neutral surfaces. `primary`, `secondary`, and `tertiary` replace the old neon green/blue/orange brand assumptions.
...
- Release notes use the shared `components/WhatsNewModal.tsx` centered dialog. Empty first-run libraries instead see `components/OnboardingModal.tsx`; the startup gate must never stack both dialogs. Content comes only from shared components/data. Stats can reopen either guide.
