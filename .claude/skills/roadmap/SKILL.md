...
- Owner verified migration survival, legacy backup import, rereads, ownership controls, APK-over-APK data survival, persistent storage, large text, and responsive desktop layout. Completed readings are protected: starting again creates a reread, while moving a completed entry back to Want requires an explicit correction warning.
